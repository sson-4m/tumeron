...
class HowToUseViewController: UIViewController {
    ...
    var backToPageTop: UIButton = UIButton()
    ...
    override func viewWillLayoutSubviews() {
        ...
        */
        ...
        outView.addSubview(backToMenuButton)
        ...
        //backToMenuButton.frame = CGRect(origin: CGPoint(x:0, y: 0), size: backToMenuButton.sizeThatFits(CGSize(width: backToMenuButton.frame.width, height: backToMenuButton.frame.width)))
        ...
        /*
        outView.addSubview(backToPageTop)
        backToPageTop.translatesAutoresizingMaskIntoConstraints = false
        backToPageTop.trailingAnchor.constraint(equalTo: backToMenuButton.trailingAnchor, constant: -20).isActive = true
        backToPageTop.bottomAnchor.constraint(equalTo: outView.bottomAnchor).isActive = true
        backToPageTop.widthAnchor.constraint(equalToConstant: self.view.frame.width / 6).isActive = true
        backToPageTop.heightAnchor.constraint(equalToConstant: self.view.frame.width / 6).isActive = true
        backToPageTop.addTarget(self, action: #selector(buttonTapped(_:)), for: UIControl.Event.touchUpInside)
        //https://icon-rainbow.com/ターン・バックするの矢印アイコン素材-2/
        backToPageTop.backgroundColor = UIColor(red: 240/255, green: 240/255, blue: 240/255, alpha: 1.0)
        backToPageTop.layer.borderWidth = 1.0
        backToPageTop.layer.borderColor = CGColor(red: 80/255, green: 80/255, blue: 80/255, alpha: 1.0)
        //backToMenuButton.setTitleColor(UIColor(red: 255/255, green: 255/255, blue: 186/255, alpha: 1.0), for: .normal)
        //backToMenuButton.setTitle("戻る", for: .normal)
        backToPageTop.setImage(UIImage(named: "returnImage2"), for: .normal)
        backToPageTop.imageView?.contentMode = .scaleAspectFill
        backToPageTop.contentHorizontalAlignment = .fill
        backToPageTop.contentVerticalAlignment = .fill
        backToPageTop.imageEdgeInsets = UIEdgeInsets(top: 10, left: 10, bottom: 10, right: 10);
        //backToMenuButton.titleLabel?.font = UIFont.systemFont(ofSize: 25)
        //backToMenuButton.titleLabel?.adjustsFontSizeToFitWidth = true
        backToPageTop.layer.cornerRadius = backToPageTop.bounds.width / 2
        */
        ...
        hTPLabel1.trailingAnchor.constraint(equalTo: contentsView.trailingAnchor, constant: -10).isActive = true
...
//        hTPLabel4.minimumScaleFactor = 0.5
        ...
        contentsView.addSubview(howToUseIV2)
...
//        hTPLabel4.minimumScaleFactor = 0.5
        ...
        contentsView.addSubview(howToUseIV3)
